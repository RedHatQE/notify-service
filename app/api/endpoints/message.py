...
async def msg_multi_tgts(
    ...
    subject: str = Query(
        f"Notification from {settings.PROJECT_NAME}", description="The message subject"
    ),
    ...
    environment: Union[
        ...
        ...,
        example={"body": "SAMPLE PLAIN TEXT MESSAGE OR JSON DICT."},
    ...
    gchat_webhook_url: Optional[AnyHttpUrl] = Query(
        None, description="The gchat webhook url"
    ),
    slack_webhook_url: Optional[AnyHttpUrl] = Query(
        None, description="The slack webhook url"
    ),
    jira_project_issue_key: str = Query(
        None,
        description="The issue key for adding a comment - i.e. CCITNOTES-50, project key for new issue - i.e. CCITNOTES",
    ),
    jira_issue_type: str = Query(
        "Task",
        enum=settings.JIRA_ISSUE_TYPE_LIST,
        description="Required for a new issue: - Issue type - i.e. Bug/Task/Story/Epic/etc.",
    ),
    ...
    bugzilla_bug_id: int = Query(None, description="The bug_id - i.e. 1997649"),
    ...
    if (
        "gchat" in target and not (gchat_webhook_url or settings.GCHAT_WEBHOOK_URL)
    ) or ("slack" in target and not (slack_webhook_url or settings.SLACK_WEBHOOK_URL)):
    ...
    if (
        "jira" in target
        and (not jira_project_issue_key[-1].isdigit())
        and (not jira_issue_type or not jira_issue_summary)
    ):
    ...
    if "irc" in target:
        ...
        await irc.send_message(channel=irc_channel, message=text)
    ...
    if "message_bus" in target:
        message_bus.send_message(topic=message_bus_topic, environment=env)
