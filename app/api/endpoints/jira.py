...
async def add_a_jira_comment(
    ...
    if not template_url and (
        ...
        or (template_name == "jira_default" and "body" not in environment.body)
    ):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to connect - please check your token/issue_key",
        )


@router.post("/new_issue", response_model=schemas.Msg)
async def create_a_jira_issue(
    project_key: str = Query(None, description="Project key - i.e. CCITNOTES"),
    issue_type: str = Query(
        "Task",
        enum=settings.JIRA_ISSUE_TYPE_LIST,
        description="Issue type - i.e. Bug/Task/Story/Epic/etc.",
    ),
    ...
    environment: Union[
        schemas.TxtBody, schemas.JiraBody, schemas.BaseResultBody
    ] = Body(
        ...,
        example={"body": "SAMPLE MESSAGE."},
        description="The body values for parse with the template, "
    ...
    if project_key is None or issue_summary is None or issue_type is None:
        raise HTTPException(
            status_code=400,
            detail="Please input values for project_key, issue_summary, issue_description, and issue_type",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to connect - please check your token/project_key",
        )
