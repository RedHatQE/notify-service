...
def test_get_template_list(client: TestClient, api_key_headers: Dict[str, str]) -> None:
...
def test_get_template(client: TestClient, api_key_headers: Dict[str, str]) -> None:
    _file_name = "default"
    r = client.get(
        f"{settings.API_V1_STR}/template/{_file_name}", headers=api_key_headers
    )
    tmplt = r.json()
    assert tmplt


def test_update_template(client: TestClient, api_key_headers: Dict[str, str]) -> None:
    ...
    try:
        ...
        r = client.put(
            f"{settings.API_V1_STR}/template/{_file_name}",
            files=_files,
